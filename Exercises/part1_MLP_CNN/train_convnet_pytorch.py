...
WORK_DIR_DEFAULT = '/home/antonioguj/Exercises_Computer_Vision_by_Learning/results/assignment_2/'
FLAGS = None
...
def train():
  ...
  size_image = train_data.images.shape[-2:]
  size_in_model = (n_channels, size_image[0], size_image[1])
  n_classes = 10 #train_data.labels[0].size

  # design model (and compiled in cuda)
  model = ConvNet(size_image=size_image, n_channels=n_channels, n_classes=n_classes)
  model.to(_device)
  summary(model, size_in_model, batch_size=batch_size, device='cuda' if _device=='cuda:0' else 'cpu')
