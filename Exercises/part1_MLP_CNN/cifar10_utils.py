...
def load_cifar10_batch(batch_filename):
  ...
  with open(batch_filename, 'rb') as f:
    #batch = pickle.load(f, encoding='latin1') #does not work with Python.2.7
    batch = pickle.load(f)
    X = batch['data']
    Y = batch['labels']
...
class DataSet(object):
  ...
  def convert_data_to_MLP_input(self, is_one_hot=True):
    self._images = self._images.reshape(self._images.shape[0], -1)
    if is_one_hot:
      self._labels = self._labels.reshape(self._images.shape[0], -1)
    else:
      self._labels = self._labels.reshape(self._images.shape[0])
